-- LESSON 2: Relational Database - Primary Key & Foreign Key - Operators - Deal with NULL

-- Part 1. Create Primary Key & Foreign Key 
...
CREATE TABLE MX_NHANVIEN (
      EmID CHAR(5)  NOT NULL PRIMARY KEY
    , EmName NVARCHAR(50)
    , Sex NVARCHAR(10)
    , TaxCode CHAR(7)
    , Email NVARCHAR(50)
...
CREATE TABLE MX_HOADON (
    ...
    , OrderLine VARCHAR(2)
    , SalesID CHAR(5) FOREIGN KEY REFERENCES MX_NHANVIEN(EmID)
    , CusID CHAR(5)
    , ProductID CHAR(6) NOT NULL FOREIGN KEY REFERENCES MX_SANPHAM(ProductID)
    , Type VARCHAR(10)
    , QTY INT
    , Price INT
    , TotalLine INT
    , DateCreate DATE
    , OrderDate DATE
    , CONSTRAINT PK_HOADON PRIMARY KEY (OrderID,ProductID)
);
...
-- Part 2: List of orders before 2022-03-04
SELECT *
FROM MX_HOADON
WHERE OrderDate < '2022-03-04';

-- Part 3.1: Employee's address in Quảng Bình và Quảng Trị 
SELECT *
FROM MX_NHANVIEN
WHERE [Address] IN (N'Quảng Bình',N'Quảng Trị ');

-- Part 3.2: List of Employee's address in Nghệ An, Hà Tĩnh, Quảng Bình, Quảng Trị và Thừa Thiên Huế, and List of the rest of Employee
SELECT *
FROM MX_NHANVIEN
...
-- Part 4: Update NULL value in StartDate column of MX_NHANVIEN table by 2020-01-01
SELECT ISNULL(StartDate, '2020-01-01') AS StartDate
FROM MX_NHANVIEN;
